...
const QuestionUpdateForm = () => {
  ...
  useEffect(() => {
    (async () =>{
      if (id) {
        ...
        setCorrectValue(response.data.correctValue);
        setDifficultyLevel(response.data.difficultyLevel);
        setSubject(response.data?.subject);
        setMarks(response.data?.marks);
        setNegativeMarks(response.data?.negativeMarks);
        setIsPYQ(response.data.isPYQ);
        setPyqDetails(response.data?.pyqDetails);
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-2xl font-semibold text-center mb-6">Update Question</h2>
