...
const ManageTestPage = () => {
  ...
  useEffect(() => {
    axios
      .get(import.meta.env.VITE_API_URL+`/api/v1/tests/${id}`, {
        withCredentials: true
      }) // Replace with your tests API endpoint
      .then((response) => {
        console.log(response.data);
        setTestDetails({
          ...
          totalMarks: response.data.totalMarks, // Initialize totalMarks as 0
          difficultyLevel: response.data.difficultyLevel,
        });
        setSelectedQuestions(response.data.subjectSections.flatMap((question) => {
